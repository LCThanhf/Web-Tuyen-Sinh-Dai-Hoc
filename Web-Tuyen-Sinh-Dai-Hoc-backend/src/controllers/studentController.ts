...
export class StudentController {
  ...
  static async upsertScore(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { type, files, ...scoreData } = req.body;

      // For the same type, find existing record to update or create new
      ...
      if (existingScore) {
        ...
        score = await prisma.score.update({
          ...
          data: {
            ...scoreData,
            files: files || existingScore.files,
            status: 'PENDING'
          }
        });
      } else {
  ...
  static async updatePriority(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { priorityArea, priorityObject, areaFile, objectFile } = req.body;
  ...
  // File Upload Methods
  static async uploadPriorityFile(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      res.json({
        success: true,
        message: 'Priority file uploaded successfully',
        data: {
          fileUrl
        }
      });
    } catch (error) {
      console.error('Upload priority file error:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to upload priority file'
      });
    }
  }

  static async uploadScoreFile(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.userId;
      const file = req.file;
      ...
      console.error('Upload score file error:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to upload score file'
      });
    }
  }
