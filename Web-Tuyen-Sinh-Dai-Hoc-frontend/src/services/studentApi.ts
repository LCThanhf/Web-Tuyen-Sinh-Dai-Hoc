...
export const studentApi = {
  ...
  uploadPriorityFile: async (file: File) => {
    ...
    formData.append('priorityFile', file);
  ...
  // Score file uploads
  uploadExamFile: async (file: File) => {
    const formData = new FormData();
    formData.append('examFile', file);

    return apiClient.post('/student/scores/upload-exam', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  uploadTranscriptFile: async (file: File) => {
    ...
    return apiClient.post('/student/scores/upload-transcript', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  uploadAssessmentFile: async (file: File) => {
    const formData = new FormData();
    formData.append('assessmentFile', file);

    return apiClient.post('/student/scores/upload-assessment', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
