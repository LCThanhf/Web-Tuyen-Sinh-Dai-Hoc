...
const Scores: React.FC = () => {
  ...
  const onExamFinish = async (values: any) => {
    try {
      ...
      // Upload exam file if provided
      let fileUrls: string[] = [];
      if (values.examFile && values.examFile.length > 0) {
        try {
          const uploadResponse = await studentApi.uploadExamFile(values.examFile[0].originFileObj);
          if (uploadResponse.data.success) {
            fileUrls.push(uploadResponse.data.data.fileUrl);
            message.success('Tải file minh chứng thành công!');
          }
        } catch (error) {
          message.error('Không thể tải file minh chứng lên. Vui lòng thử lại.');
          return;
        }
      }
  ...
  const onHkbFinish = async () => {
    try {
      ...
      let fileUrls: string[] = [];
      if (values.hkbFile && values.hkbFile.length > 0) {
        try {
          const uploadResponse = await studentApi.uploadTranscriptFile(values.hkbFile[0].originFileObj);
          if (uploadResponse.data.success) {
            fileUrls.push(uploadResponse.data.data.fileUrl);
            message.success('Tải file minh chứng thành công!');
          }
  ...
  const onDgnlDgtdFinish = async (values: any) => {
    try {
      ...
      if (noScoreDeclared) {
        ...
        // Upload assessment file if provided
        let fileUrls: string[] = [];
        if (assessmentFile && assessmentFile.length > 0) {
          try {
            const uploadResponse = await studentApi.uploadAssessmentFile(assessmentFile[0].originFileObj);
            if (uploadResponse.data.success) {
              fileUrls.push(uploadResponse.data.data.fileUrl);
              message.success('Tải file minh chứng thành công!');
            }
