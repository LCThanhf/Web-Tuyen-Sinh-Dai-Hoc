...
const AdminManageApplicationsPage: React.FC = () => {
  ...
  const getCombinationName = (id: string) => {
    console.log('🔍 Looking for combination ID:', id);
    console.log('📚 Available combinations:', combinations.map(c => `${c.id}: ${c.name}`));
    const combination = combinations.find(c => c.id === id);
    console.log('✅ Found combination:', combination);
    return combination?.name || 'N/A';
  };
  ...
  const columns: ColumnType<StudentApplication>[] = [
    ...
    },
    {
      ...
      render: (_: any, record: StudentApplication) => {
        console.log('🎯 Rendering combo/unit for record:', {
          id: record.id,
          admissionMethod: record.admissionMethod,
          combinationId: record.combinationId,
          organizingUnit: record.organizingUnit
        });
        
        // Check if it's a score-based method (THPT or Học bạ)
        const isScoreBased = record.admissionMethod === 'Điểm THPT' || 
                            record.admissionMethod === 'Học bạ' ||
                            record.admissionMethod === 'Điểm THPT / Học bạ';
        
        if (isScoreBased && record.combinationId) {
          const comboName = getCombinationName(record.combinationId);
          console.log('📚 Combination name result:', comboName);
          return comboName;
        } else if (!isScoreBased && record.organizingUnit) {
          console.log('🏢 Using organizing unit:', record.organizingUnit);
          return record.organizingUnit;
        } else {
          console.log('⚠️ No combination or organizing unit found');
          return 'N/A';
        }
      }
    },
      ...
      >
        ...
        >
          ...
          {(form.getFieldValue('admissionMethod') === 'Điểm THPT / Học bạ' || 
            form.getFieldValue('admissionMethod') === 'Điểm THPT' || 
            form.getFieldValue('admissionMethod') === 'Học bạ') && (
            <Form.Item
              name="combinationId"
          ...
          {(form.getFieldValue('admissionMethod') === 'Đánh giá năng lực / Tư duy' ||
            form.getFieldValue('admissionMethod') === 'Đánh giá năng lực/Đánh giá tư duy') && (
            <Form.Item
              name="organizingUnit"
