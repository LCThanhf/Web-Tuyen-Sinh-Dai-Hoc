...
const Scores: React.FC = () => {
  ...
  const [dgnlDgtdScoresTableData, setDgnlDgtdScoresTableData] = useState<DgnlDgtdScoreEntry | null>(null);
  ...
  const handleEditExamScore = () => {
    if (examScoresTableData) {
      ...
      examForm.setFieldsValue({
        ...
        ...Object.keys(examScoresTableData.scores).reduce((acc, subj) => {
  ...
  const onDgnlDgtdFinish = (values: any) => {
    ...
    setDgnlDgtdScoresTableData(newEntry); // Store as a single entry
  ...
  const handleEditDgnlDgtdScore = () => {
    if (dgnlDgtdScoresTableData) {
      dgnlDgtdForm.setFieldsValue({
        assessmentType: dgnlDgtdScoresTableData.type,
        assessmentUnit: dgnlDgtdScoresTableData.assessmentUnit,
        assessmentScore: dgnlDgtdScoresTableData.assessmentScore,
        assessmentFile: dgnlDgtdScoresTableData.assessmentFile.length > 0 ? [{ uid: '-1', name: 'uploaded_file', status: 'done', url: URL.createObjectURL(dgnlDgtdScoresTableData.assessmentFile[0]) }] : [],
      });
      setSelectedAssessmentType(dgnlDgtdScoresTableData.type);
      setCurrentEditingDgnlDgtdKey(dgnlDgtdScoresTableData.key);
      message.info("Bạn đang chỉnh sửa điểm ĐGNL/ĐGTD.");
    }
  };

  const handleDeleteDgnlDgtdScore = () => {
    setDgnlDgtdScoresTableData(null);
    ...
    },
    {
      ...
      render: (_: any, record: DgnlDgtdScoreEntry) => (
        <Space size="middle">
          <Button
            icon={<EditOutlined />}
            onClick={handleEditDgnlDgtdScore}
            disabled={record.status !== "Chờ duyệt"}
          >
            Sửa
          </Button>
          <Popconfirm
            title="Bạn có chắc chắn muốn xóa điểm này?"
            onConfirm={handleDeleteDgnlDgtdScore}
            okText="Có"
            cancelText="Không"
          >
            <Button
    ...
    <Tabs defaultActiveKey="1" centered size="large" style={{ padding: "20px" }}>
      ...
      <TabPane tab="Điểm thi THPT" key="1">
        ...
        >
          ...
          <Form.Item>
            <Button type="primary" htmlType="submit" block disabled={!!examScoresTableData && !currentEditingExamKey}>
      ...
      <TabPane tab="Điểm học bạ" key="2">
        ...
        >
          ...
          <Form.Item>
            <Button type="primary" htmlType="submit" block disabled={!!hkbScoresTableData && !currentEditingHkbKey}>
      ...
      <TabPane tab="Đánh giá năng lực/tư duy" key="3">
        <Form
          ...
          style={{ maxWidth: 700, margin: "auto", padding: "20px", border: "1px solid #f0f0f0", borderRadius: "8px" }}
        >
          <Title level={4} style={{ textAlign: "center", marginBottom: "24px" }}>Nhập điểm Đánh giá năng lực/tư duy</Title>
          ...
          {selectedAssessmentType && (
            ...
            >
              <Select placeholder="Chọn đơn vị tổ chức">
                ...
                {selectedAssessmentType === "ĐGTD" && assessmentUnits.dgtd.map(({ label, value }) => (
                  <Option key={value} value={value}>
                    {label}
                  </Option>
                ))}
          ...
          <Form.Item>
            <Button type="primary" htmlType="submit" block disabled={!!dgnlDgtdScoresTableData && !currentEditingDgnlDgtdKey}>
