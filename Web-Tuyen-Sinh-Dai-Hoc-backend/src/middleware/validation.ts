...
export const validateAdminSchool = [
  body('name')
    .trim()
  ...
  body('code')
    ...
    .matches(/^[A-Z0-9_-]+$/)
    .withMessage('School code must be 2-20 characters, uppercase letters, numbers, underscores, and hyphens only'),
  body('totalQuota')
    .isInt({ min: 1 })
    .withMessage('Total quota must be a positive integer'),
  body('admissionMethods')
    .isArray({ min: 1 })
    .withMessage('At least one admission method is required'),
  body('admissionMethods.*.name')
    .trim()
    .notEmpty()
    .withMessage('Admission method name is required'),
  body('admissionMethods.*.percentage')
    .isFloat({ min: 0, max: 100 })
    .withMessage('Admission method percentage must be between 0 and 100'),
  handleValidationErrors
];

export const validateAdminMajor = [
  body('schoolId')
    .isUUID()
  ...
  body('name')
    .trim()
    .isLength({ min: 2, max: 200 })
    .withMessage('Major name must be 2-200 characters'),
  body('code')
    .trim()
    .isLength({ min: 2, max: 20 })
    .matches(/^[A-Z0-9_-]+$/)
    .withMessage('Major code must be 2-20 characters, uppercase letters, numbers, underscores, and hyphens only'),
  body('quota')
    .isInt({ min: 1 })
...
export const validateAdminCombination = [
  body('name')
    .trim()
    .isLength({ min: 2, max: 100 })
    .withMessage('Combination name must be 2-100 characters'),
  body('subjects')
    .isArray({ min: 1 })
