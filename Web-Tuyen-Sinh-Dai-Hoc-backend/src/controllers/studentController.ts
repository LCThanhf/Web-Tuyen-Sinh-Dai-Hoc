...
export class StudentController {
  ...
  static async uploadExamFile(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.userId;
      const file = req.file;
      ...
      // If scoreId is provided, update existing score, otherwise create/update THPT score
      let score;
      if (scoreId) {
        ...
        if (!existingScore || existingScore.studentId !== student.id) {
          res.status(404).json({
            success: false,
        ...
        if (existingScore.files && Array.isArray(existingScore.files)) {
          updatedFiles = [...(existingScore.files as string[])];
        }
        ...
        // Find or create THPT score record
        const existingThptScore = await prisma.score.findFirst({
          where: {
            studentId: student.id,
            type: 'THPT'
          }
        ...
        if (existingThptScore) {
          let updatedFiles: string[] = [];
          if (existingThptScore.files && Array.isArray(existingThptScore.files)) {
            updatedFiles = [...(existingThptScore.files as string[])];
          }
          updatedFiles.push(fileUrl);

          score = await prisma.score.update({
            where: { id: existingThptScore.id },
            data: {
              files: updatedFiles,
              status: 'PENDING'
            }
  ...
  static async uploadTranscriptFile(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.userId;
      const file = req.file;
      const { scoreId } = req.body;
      ...
      // If scoreId is provided, update existing score, otherwise create/update TRANSCRIPT score
      let score;
      if (scoreId) {
        const existingScore = await prisma.score.findUnique({
          where: { id: scoreId }
        ...
        // Find or create TRANSCRIPT score record
        const existingTranscriptScore = await prisma.score.findFirst({
          where: {
            studentId: student.id,
            type: 'TRANSCRIPT'
          }
        ...
        if (existingTranscriptScore) {
          let updatedFiles: string[] = [];
          if (existingTranscriptScore.files && Array.isArray(existingTranscriptScore.files)) {
            updatedFiles = [...(existingTranscriptScore.files as string[])];
          }
          updatedFiles.push(fileUrl);

          score = await prisma.score.update({
            where: { id: existingTranscriptScore.id },
            data: {
              files: updatedFiles,
              status: 'PENDING'
            }
  ...
  static async uploadAssessmentFile(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.userId;
      const file = req.file;
      const { scoreId } = req.body;
      ...
      // If scoreId is provided, update existing score, otherwise create/update ASSESSMENT score
      let score;
      if (scoreId) {
        const existingScore = await prisma.score.findUnique({
          where: { id: scoreId }
        ...
        // Find or create ASSESSMENT score record
        const existingAssessmentScore = await prisma.score.findFirst({
          where: {
            studentId: student.id,
            type: 'ASSESSMENT'
          }
        ...
        if (existingAssessmentScore) {
          let updatedFiles: string[] = [];
          if (existingAssessmentScore.files && Array.isArray(existingAssessmentScore.files)) {
            updatedFiles = [...(existingAssessmentScore.files as string[])];
          }
          updatedFiles.push(fileUrl);

          score = await prisma.score.update({
            where: { id: existingAssessmentScore.id },
            data: {
              files: updatedFiles,
              status: 'PENDING'
            }
