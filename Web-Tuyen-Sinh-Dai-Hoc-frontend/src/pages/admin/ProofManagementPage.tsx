  ...
  const getColumns = (): any[] => {
    ...
    if (currentTab === 'scores') {
      ...
      baseColumns.splice(6, 0, {
        ...
        key: 'proofFiles',
        render: (_: any, record: any) => {
          const fileUrls = record.fileUrls || [];
                ...
                <Button 
                  ...
                  onClick={() => {
                    const typeLabels = {
                      'THPT': 'thi_thpt',
                      'TRANSCRIPT': 'hoc_ba',
                      'ASSESSMENT': 'danh_gia_nl'
                    };
                    const typeLabel = typeLabels[record.type as keyof typeof typeLabels] || 'diem';
                    handleViewFile(fileUrl, `${typeLabel}_${record.student?.user?.fullName || 'file'}_${index + 1}`);
                  }}
                  size="small"
                >
                  Xem file
        ...
        render: (fileUrl: string, record: any) =>
          fileUrl ? (
            <Button 
              type="link" 
              icon={<EyeOutlined />} 
              onClick={() => handleViewFile(fileUrl, `ưu_tiên_kv_${record.student?.user?.fullName || 'khu_vuc'}`)}
              size="small"
            >
              Xem file
            </Button>
        ...
        render: (fileUrl: string, record: any) =>
          fileUrl ? (
            <Button 
              type="link" 
              icon={<EyeOutlined />} 
              onClick={() => handleViewFile(fileUrl, `ưu_tiên_dt_${record.student?.user?.fullName || 'doi_tuong'}`)}
              size="small"
            >
              Xem file
            </Button>
        ...
        <Row gutter={[16, 16]}>
          ...
          {currentTab === 'scores' && (
            <>
              ...
              {/* Files Section for Scores */}
              {(selectedDocument.fileUrls && selectedDocument.fileUrls.length > 0) && (
                <Col span={24}>
                  <Title level={5} style={{ marginTop: 16, marginBottom: 12 }}>File minh chứng điểm</Title>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {selectedDocument.fileUrls.map((fileUrl: string, index: number) => (
                      <Button 
                        key={index}
                        type="link" 
                        icon={<EyeOutlined />} 
                        onClick={() => {
                          ...
                          const typeLabel = typeLabels[selectedDocument.type as keyof typeof typeLabels] || 'diem';
                          handleViewFile(fileUrl, `${typeLabel}_${selectedDocument.student?.user?.fullName || 'file'}_${index + 1}`);
                        }}
                      >
                        Xem file {index + 1}
                      </Button>
          ...
          {currentTab === 'priority' && (
            <>
              ...
              {/* Files Section for Priority */}
              {(selectedDocument.areaFile || selectedDocument.objectFile) && (
                <Col span={24}>
                  <Title level={5} style={{ marginTop: 16, marginBottom: 12 }}>File minh chứng ưu tiên</Title>
                </Col>
              )}
              {selectedDocument.areaFile && (
                <Col span={12}>
                  <Text strong>Minh chứng khu vực: </Text>
                  <Button 
                    type="link" 
                    icon={<EyeOutlined />} 
                    onClick={() => handleViewFile(selectedDocument.areaFile, `ưu_tiên_kv_${selectedDocument.student?.user?.fullName || 'khu_vuc'}`)}
                  >
                    Xem file minh chứng KV
                  </Button>
                </Col>
              )}
              {selectedDocument.objectFile && (
                <Col span={12}>
                  <Text strong>Minh chứng đối tượng: </Text>
                  <Button 
                    type="link" 
                    icon={<EyeOutlined />} 
                    onClick={() => handleViewFile(selectedDocument.objectFile, `ưu_tiên_dt_${selectedDocument.student?.user?.fullName || 'doi_tuong'}`)}
                  >
                    Xem file minh chứng ĐT
                  </Button>
                </Col>
              )}
            </>
